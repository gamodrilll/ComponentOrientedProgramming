 ...
 */
public class HackAssembler extends Translators.HackTranslator {
    ...
    public HackAssembler(String fileName, int size, short nullValue, boolean save)
     throws Translators.HackTranslatorException {
    ...
    // if not legal.
    private void checkComparisonFile(String fileName) throws Translators.HackTranslatorException {
        if (!fileName.endsWith("." + getDestinationExtension()))
            throw new Translators.HackTranslatorException(fileName + " is not a ." + getDestinationExtension()
                                              + " file");

        File file = new File(fileName);
        if (!file.exists())
            throw new Translators.HackTranslatorException("File " + fileName + " does not exist");
    }

    protected void restartCompilation() {
        super.restartCompilation();

        varIndex = Utilities.Definitions.VAR_START_ADDRESS;
    ...
    private void resetComparisonFile() throws Translators.HackTranslatorException {
        try {
            comparisonReader = new BufferedReader(new FileReader(comparisonFileName));

            if (gui != null) {
                ComputerParts.TextFileGUI comp = ((HackAssemblerGUI)gui).getComparison();
                comp.reset();
                ...
                for (int i = 0; i < comp.getNumberOfLines(); i++) {
					if (comp.getLineAt(i).length() != Utilities.Definitions.BITS_PER_WORD) {
						throw new Translators.HackTranslatorException("Error in file "+comparisonFileName+": Line "+i+" does not contain exactly "+ Utilities.Definitions.BITS_PER_WORD+" characters");
					}
					try {
						comparisonProgram[i] = (short) Utilities.Conversions.binaryToInt(comp.getLineAt(i));
					} catch (NumberFormatException nfe) {
						throw new Translators.HackTranslatorException("Error in file "+comparisonFileName+": Line "+i+" does not contain only 1/0 characters");
					}
				}
            }
        } catch (IOException ioe) {
            throw new Translators.HackTranslatorException("Error reading from file " + comparisonFileName);
        }
    }

    protected void initSource() throws Translators.HackTranslatorException {
        generateSymbolTable();
    }

    // Generates The symbol table by attaching each label with it's appropriate
    // value according to it's location in the program
    private void generateSymbolTable() throws Translators.HackTranslatorException {
        symbolTable = Utilities.Definitions.getInstance().getAddressesTable();
        short pc = 0;
        String line;
        String label;
        ...
        try {
            ...
            throw new Translators.HackTranslatorException("Error reading from file " + sourceFileName);
        }
    }

    protected void initCompilation() throws Translators.HackTranslatorException {
        if (gui != null && (inFullCompilation || !compilationStarted))
            ((HackAssemblerGUI)gui).disableLoadComparison();
    }

    protected void successfulCompilation() throws Translators.HackTranslatorException {
    ...
    protected int[] compileLineAndCount(String line) throws Translators.HackTranslatorException {
        ...
        if (compiledRange != null && comparisonReader != null) {
            ...
            if (!compare)
                throw new Translators.HackTranslatorException("Comparison failure");
        }
    ...
    protected String getCodeString(short code, int pc, boolean display) {
        return Utilities.Conversions.decimalToBinary(code, 16);
    }
    ...
    protected void compileLine(String line) throws Translators.HackTranslatorException {
        ...
        try {
            ...
            throw new Translators.HackTranslatorException(ae.getMessage(), sourcePC);
        }
    }
    ...
    public void rowSelected(ComputerParts.TextFileEvent event) {
    ...
    public void actionPerformed(Translators.HackTranslatorEvent event) {
        super.actionPerformed(event);

        switch (event.getAction()) {
            case Translators.HackTranslatorEvent.SOURCE_LOAD:
                comparisonFileName = "";
            ...
            case HackAssemblerEvent.COMPARISON_LOAD:
                ...
                } catch (Translators.HackTranslatorException ae) {
