...
public class ScreenComponent extends JPanel implements ScreenGUI, ActionListener {
    ...
    public ScreenComponent() {
        ...
        setPreferredSize(new Dimension(Utilities.Definitions.SCREEN_WIDTH + borderWidth,
                                       Utilities.Definitions.SCREEN_HEIGHT + borderHeight));
        setSize(Utilities.Definitions.SCREEN_WIDTH + borderWidth,
                Utilities.Definitions.SCREEN_HEIGHT + borderHeight);

        data = new short[Utilities.Definitions.SCREEN_SIZE];
        x = new int[Utilities.Definitions.SCREEN_SIZE];
        y = new int[Utilities.Definitions.SCREEN_SIZE];
        x[0] = borderInsets.left;
        y[0] = borderInsets.top;

        // updates pixels indice
        for (int i = 1; i < Utilities.Definitions.SCREEN_SIZE; i++) {
            x[i] = x[i - 1] + Utilities.Definitions.BITS_PER_WORD;
            y[i] = y[i - 1];
            if (x[i] == Utilities.Definitions.SCREEN_WIDTH + borderInsets.left) {
    ...
    public void paintComponent(Graphics g) {
        ...
        for (int i = 0; i < Utilities.Definitions.SCREEN_SIZE; i++) {
            if (data[i] != 0) {
