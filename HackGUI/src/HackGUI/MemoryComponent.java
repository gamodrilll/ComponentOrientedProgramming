 ...
 */
public class MemoryComponent extends JPanel implements ComputerParts.MemoryGUI {
    ...
    public void addListener(ComputerParts.ComputerPartEventListener listener) {
        listeners.addElement(listener);
    }

    public void removeListener(ComputerParts.ComputerPartEventListener listener) {
        listeners.removeElement(listener);
    }

    public void notifyListeners(int address, short value) {
        ComputerParts.ComputerPartEvent event = new ComputerParts.ComputerPartEvent(this,address,value);
        for (int i=0;i<listeners.size();i++) {
           ((ComputerParts.ComputerPartEventListener)listeners.elementAt(i)).valueChanged(event);
        }
    }

    public void notifyListeners() {
        ComputerParts.ComputerPartEvent event = new ComputerParts.ComputerPartEvent(this);
        for (int i=0;i<listeners.size();i++) {
           ((ComputerParts.ComputerPartEventListener)listeners.elementAt(i)).guiGainedFocus();
        }
    }

    public void addClearListener (Events.ClearEventListener listener) {
        clearListeners.addElement(listener);
    }
    public void removeClearListener (Events.ClearEventListener listener) {
        clearListeners.removeElement(listener);
    }

    public void notifyClearListeners() {
        Events.ClearEvent clearEvent = new Events.ClearEvent(this);
        for(int i=0; i<clearListeners.size();i++)
            ((Events.ClearEventListener)clearListeners.elementAt(i)).clearRequested(clearEvent);
    }

    /**
     * Registers the given ErrorEventListener as a listener to this GUI.
     */
    public void addErrorListener(Events.ErrorEventListener listener) {
     ...
     */
    public void removeErrorListener(Events.ErrorEventListener listener) {
   ...
   /**
    ...
    public void notifyErrorListeners(String errorMessage) {
        Events.ErrorEvent event = new Events.ErrorEvent(this, errorMessage);
        for (int i=0; i<errorEventListeners.size(); i++)
            ((Events.ErrorEventListener)errorEventListeners.elementAt(i)).errorOccured(event);
    }
