 ...
 */
public class HardwareSimulator extends Controller.HackSimulator
 implements ComputerParts.TextFileEventListener, GateErrorEventListener,
            DirtyGateListener {
    ...
    public String getValue(String varName) throws Controller.VariableException {
        ...
        if (gate == null)
            throw new Controller.VariableException("cannot get var's value since no gate is currently loaded", varName);
                ...
                if (gateName != null) {
                    ...
                    if (guiChip != null) {
                        try {
                            ...
                            throw new Controller.VariableException(ge.getMessage(), varName);
                        }
                    } else {
						throw new Controller.VariableException("No such built-in chip used",
													gateName);
					}
                }
    ...
    private int getVarIndex(String varName) throws Controller.VariableException {
        int index;
        ...
        if (varName.endsWith("[]"))
            ...
            try {
                ...
                throw new Controller.VariableException("Illegal component index", varName);
            }
        }
            ...
            if (node != null) {
                ...
                if (type != GateClass.INPUT_PIN_TYPE)
                    ...
                    if (!isLegalWidth(varName, numValue))
                        throw new Controller.VariableException(value + " doesn't fit in the pin's width",
                                                    varName);
                    else
                        node.set(numValue);
                }
     ...
     throws Controller.CommandException, Controller.ProgramException, Controller.VariableException {
        ...
        if (command[0].equals(COMMAND_TICK)) {
            if (command.length != 1)
                ...
                throw new Controller.CommandException("Illegal command since no gate is currently loaded", command);
            else if (clockUp)
                throw new Controller.CommandException("Illegal command since clock is already up", command);
            ...
            if (command.length != 1)
                ...
                throw new Controller.CommandException("Illegal command since clock is already down", command);
            ...
            try {
                if (!command[1].endsWith(".hdl"))
                    throw new Controller.CommandException("A .hdl file is expected", command);

                if (command[1].indexOf("/") >= 0)
                    throw new Controller.CommandException("The gate name must not contain path specification",
                                               command);

                // use gate name without the .hdl extension
                String gateName = command[1].substring(0, command[1].length() - 4);
                loadGate(gateName, false);
                notifyProgramListeners(Events.ProgramEvent.LOAD, GatesManager.getInstance().getHDLFileName(gateName));
            } catch (GateException ge) {
                throw new Controller.CommandException(ge.getMessage(), command);
            }
        }
        else {
            boolean found = false;
            ...
            if (!found)
                throw new Controller.CommandException("Unknown command or component name", command);
        }
    }
    ...
    private static short getIndex(String varName) throws Controller.VariableException {
