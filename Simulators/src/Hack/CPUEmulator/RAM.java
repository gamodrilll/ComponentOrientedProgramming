 ...
 */
public class RAM extends ComputerParts.PointedMemory
{
    // The amount of miliseconds that a label should flash.
    private static final int LABEL_FLASH_TIME = 500;

    private static final short[] emptyScreen = new short[Utilities.Definitions.SCREEN_SIZE_IN_WORDS];
     ...
     */
    public RAM(ComputerParts.PointedMemoryGUI mainGUI, ComputerParts.MemorySegment[][] segments, ScreenGUI screenGUI) {
        super(Utilities.Definitions.RAM_SIZE, mainGUI);
    ...
    public void setValueAt(int address, short value, boolean quiet) {
        ...
        if (screen != null && address >= Utilities.Definitions.SCREEN_START_ADDRESS
             && address < Utilities.Definitions.SCREEN_START_ADDRESS + Utilities.Definitions.SCREEN_SIZE_IN_WORDS)
            screen.setValueAt((short)(address - Utilities.Definitions.SCREEN_START_ADDRESS), value);
        ...
        if (segments != null && segments[address] != null) {
            ...
            for (int i = 0; i < segments[address].length; i++) {
                ...
                if (segments[address][i] instanceof ComputerParts.PointedMemorySegment)
                    ((ComputerParts.PointedMemorySegment)segments[address][i]).setPointerAddress(value);
                else
    ...
    public synchronized void setLabel(int address, String name, boolean quiet) {
        if (hasGUI && gui instanceof ComputerParts.LabeledPointedMemoryGUI) {
            ((ComputerParts.LabeledPointedMemoryGUI)gui).setLabel(address, name);
            if (!quiet) {
                ((ComputerParts.LabeledPointedMemoryGUI)gui).labelFlash(address);
                try {
                    wait(LABEL_FLASH_TIME);
                } catch (InterruptedException ie) {}
                ((ComputerParts.LabeledPointedMemoryGUI)gui).hideLabelFlash();
            }
        }
    }
    ...
    public void clearLabels() {
        if (hasGUI && gui instanceof ComputerParts.LabeledPointedMemoryGUI)
            ((ComputerParts.LabeledPointedMemoryGUI)gui).clearLabels();
    }
    ...
    public void clearScreen() {
        setContents(emptyScreen, Utilities.Definitions.SCREEN_START_ADDRESS);
            ...
            for (int address = 0; address < size; address++) {
                if (segments[address] != null) {
                    for (int i = 0; i < segments[address].length; i++) {
                        // check if the relevant memory segment is a pointed one.
                        if (segments[address][i] instanceof ComputerParts.PointedMemorySegment)
                            ((ComputerParts.PointedMemorySegment)segments[address][i]).setPointerAddress(mem[address]);
                        else
