 ...
 */
public class CPUEmulator extends Controller.HackSimulator implements ComputerParts.ComputerPartErrorEventListener {
    ...
    public CPUEmulator() {
        ...
        ComputerParts.Register D = new ComputerParts.Register(null);
        D.reset();
    ...
    public CPUEmulator(CPUEmulatorGUI gui) {
        ...
        ComputerParts.Register D = new ComputerParts.Register(gui.getD());
    ...
    private static short getRomIndex(String varName) throws Controller.VariableException {
        if (varName.indexOf("]") == -1)
            throw new Controller.VariableException("Missing ']'", varName);

        String indexStr = varName.substring(varName.indexOf("[") + 1, varName.indexOf("]"));
        int index = Integer.parseInt(indexStr);
        if (index < 0 || index >= Utilities.Definitions.ROM_SIZE)
            throw new Controller.VariableException("Illegal variable index", varName);
    ...
    private void check_ram_address(String varName, int value) throws Controller.VariableException {
    ...
    // Checks that the given value is a legal 16-bit address
    private void check_rom_address(String varName, int value) throws Controller.VariableException {
        if (value < 0 || value >= Utilities.Definitions.ROM_SIZE)
            throw new Controller.VariableException(value +
                " is an illegal value for", varName);
    }
}
