 ...
 */
public class VMProgram extends ComputerParts.InteractiveComputerPart
 implements Events.ProgramEventListener {
     ...
     */
    public void loadProgram(String fileName) throws Controller.ProgramException {
        File file = new File(fileName);
        if (!file.exists())
            throw new Controller.ProgramException("cannot find " + fileName);

        File[] files;

        if (file.isDirectory()) {
            files = file.listFiles(new Utilities.HackFileFilter(".vm"));
            if (files == null || files.length == 0)
                throw new Controller.ProgramException("No vm files found in " + fileName);
        }
        else
            files = new File[]{file};
        ...
        for (int i = 0; i < files.length; i++) {
            ...
            try {
                ...
                if (displayChanges)
                    gui.hideMessage();
                throw new Controller.ProgramException(name + ": " + pe.getMessage());
            }
        }
        ...
        // Second scan
        nextPC = 0;
        currentStaticIndex = Utilities.Definitions.VAR_START_ADDRESS;
        for (int i = 0; i < files.length; i++) {
            String name = files[i].getName();
            String className = name.substring(0, name.indexOf("."));
    ...
    private void updateSymbolTable(File file, Hashtable symbols, Hashtable functions) throws Controller.ProgramException {
        BufferedReader reader = null;

        try {
            reader = new BufferedReader(new FileReader(file.getAbsolutePath()));
        } catch (FileNotFoundException fnfe) {
            throw new Controller.ProgramException("file " + file.getName() + " does not exist");
        }
        ...
        try {
                ...
                if (!line.trim().equals("")) {
                    if (line.startsWith("function ")) {
                        ...
                        if (symbols.containsKey(currentFunction))
                            throw new Controller.ProgramException("subroutine " + currentFunction +
                                                       " already exists");
            ...
            throw new Controller.ProgramException("Error while reading from file");
        } catch (NoSuchElementException nsee) {
            throw new Controller.ProgramException("In line " + lineNumber + ": unexpected end of command");
        }
		if (isSlashStar) {
			throw new Controller.ProgramException("Unterminated /* comment at end of file");
		}
    }
    ...
    private void buildProgram(File file, Hashtable symbols)
        ...
        try {
            ...
            throw new Controller.ProgramException("file does not exist");
        }
        ...
        try {
            while ((line = unCommentLine(reader.readLine())) != null) {
                ...
                if (!line.trim().equals("")) {
                    ...
                    if (opCode == HVMInstructionSet.UNKNOWN_INSTRUCTION)
                        throw new Controller.ProgramException("in line " + lineNumber +
                    ...
                    switch (opCode) {
                        case HVMInstructionSet.PUSH_CODE:
                            ...
                            try {
                                ...
                                throw new Controller.ProgramException("in line " + lineNumber + pe.getMessage());
                            }
                        ...
                        case HVMInstructionSet.CALL_CODE:
							...
							try {
								...
								throw new Controller.ProgramException("in line " +
														   lineNumber + ": " +
														   pe.getMessage());
							}
                            arg1 = Short.parseShort(tokenizer.nextToken());

                            if (arg1 < 0 || ((arg0 < 0 || arg0 > Utilities.Definitions.ROM_SIZE) && arg0 != BUILTIN_FUNCTION_ADDRESS))
                                throw new Controller.ProgramException("in line " + lineNumber +
                                                           ": Illegal argument - " + line);
                        ...
                        case HVMInstructionSet.GOTO_CODE:
                            ...
                            if (arg0 < 0 || arg0 > Utilities.Definitions.ROM_SIZE)
                                throw new Controller.ProgramException("in line " + lineNumber +
                                                           ": Illegal argument - " + line);
            ...
            throw new Controller.ProgramException("Illegal 16-bit value");
        } catch (NoSuchElementException nsee) {
            throw new Controller.ProgramException("In line " + lineNumber + ": unexpected end of command");
        }
		if (isSlashStar) {
	...
	public short getAddress(String functionName) throws Controller.ProgramException {
		...
		if (address != null) {
			...
			throw new Controller.ProgramException(className + ".vm not found " +
     ...
     */
    public ComputerParts.ComputerPartGUI getGUI() {
        return gui;
    }
    ...
    private byte translateSegment(String segment, HVMInstructionSet instructionSet,
     ...
     throws Controller.ProgramException {
        ...
        if (code == HVMInstructionSet.UNKNOWN_SEGMENT)
            throw new Controller.ProgramException(": Illegal memory segment - "
                                       + segment);
