...
public class CPU {
    ...
    // The bus.
    private ComputerParts.Bus bus;

    // The memory segments
    private ComputerParts.AbsolutePointedMemorySegment stackSegment;
    private ComputerParts.TrimmedAbsoluteMemorySegment workingStackSegment;
    private ComputerParts.MemorySegment staticSegment;
    private ComputerParts.MemorySegment localSegment;
    private ComputerParts.MemorySegment argSegment;
    private ComputerParts.MemorySegment thisSegment;
    private ComputerParts.MemorySegment thatSegment;
    private ComputerParts.MemorySegment tempSegment;

    // A mapping from memory segment codes to the MemorySegment objects (not including stack).
    private ComputerParts.MemorySegment[] segments;
    ...
    public CPU(VMProgram program, RAM ram, CallStack callStack,
			   Calculator calculator, ComputerParts.Bus bus,
               ComputerParts.AbsolutePointedMemorySegment stackSegment,
               ComputerParts.TrimmedAbsoluteMemorySegment workingStackSegment,
			   ComputerParts.MemorySegment staticSegment, ComputerParts.MemorySegment localSegment,
			   ComputerParts.MemorySegment argSegment, ComputerParts.MemorySegment thisSegment,
               ComputerParts.MemorySegment thatSegment, ComputerParts.MemorySegment tempSegment,
			   File builtInDir) {
        ...
        segments = new ComputerParts.MemorySegment[HVMInstructionSet.NUMBER_OF_ACTUAL_SEGMENTS];
     ...
     */
    public void boot() {
        stackSegment.setStartAddress(Utilities.Definitions.STACK_START_ADDRESS);
        workingStackSegment.setStartAddress(Utilities.Definitions.STACK_START_ADDRESS);
        localSegment.setEnabledRange(Utilities.Definitions.STACK_START_ADDRESS,
                                     Utilities.Definitions.STACK_END_ADDRESS, true);
        argSegment.setEnabledRange(Utilities.Definitions.STACK_START_ADDRESS,
                                   Utilities.Definitions.STACK_END_ADDRESS, true);
        thisSegment.setEnabledRange(Utilities.Definitions.HEAP_START_ADDRESS,
									Utilities.Definitions.HEAP_END_ADDRESS, true);
        thatSegment.setEnabledRange(Utilities.Definitions.HEAP_START_ADDRESS,
			   						Utilities.Definitions.SCREEN_END_ADDRESS, true);
        staticSegment.setStartAddress(Utilities.Definitions.VAR_START_ADDRESS);
        staticSegment.setEnabledRange(Utilities.Definitions.VAR_START_ADDRESS,
			   						  Utilities.Definitions.VAR_END_ADDRESS - 1, true);
        setSP(Utilities.Definitions.STACK_START_ADDRESS);
		stackFrames.clear();
     ...
     * Returns the bus.
     */
    public ComputerParts.Bus getBus() {
        return bus;
    }
     ...
     */
    public ComputerParts.MemorySegment[] getMemorySegments() {
        return segments;
    }
     ...
     */
    public ComputerParts.PointedMemorySegment getStack() {
        return stackSegment;
    }
     ...
     */
    public ComputerParts.PointedMemorySegment getWorkingStack() {
     ...
     */
    public void executeInstruction() throws Controller.ProgramException {
        currentInstruction = program.getNextInstruction();

        if (currentInstruction == null)
            throw new Controller.ProgramException("No more instructions to execute");
        ...
        switch (currentInstruction.getOpCode()) {
            ...
            case HVMInstructionSet.FUNCTION_CODE:
                if (program.getCurrentPC() == program.getPreviousPC() + 1)
                    throw new Controller.ProgramException("Missing return in " + callStack.getTopFunction());
     ...
     */
    public void add() throws Controller.ProgramException {
     ...
     */
    public void substract() throws Controller.ProgramException {
     ...
     */
    public void negate() throws Controller.ProgramException {
     ...
     */
    public void equal() throws Controller.ProgramException {
     ...
     * 0xFFFF as true,0x0000 as false
     */
    public void greaterThan() throws Controller.ProgramException {
     ...
     * 0xFFFF as true,0x0000 as false
     */
    public void lessThan() throws Controller.ProgramException {
     ...
     */
    public void and() throws Controller.ProgramException {
     ...
     */
    public void or() throws Controller.ProgramException {
     ...
     */
    public void not() throws Controller.ProgramException {
     ...
     */
    public void push(short segment, short n) throws Controller.ProgramException {
        if (segment == HVMInstructionSet.CONST_SEGMENT_CODE)
            ...
            switch (n) {
                case 0: pushFromRAM(METHOD_STACK, Utilities.Definitions.THIS_POINTER_ADDRESS);
                        break;
                case 1: pushFromRAM(METHOD_STACK, Utilities.Definitions.THAT_POINTER_ADDRESS);
                        break;
            }
        else
     ...
     */
    public void pop(short segment, short n) throws Controller.ProgramException {
     ...
     * is not zero.
     */
    public void ifGoTo(short address) throws Controller.ProgramException {
        if (popAndReturn() != 0) {
            program.setPC(address);
        }
    }
     ...
     */
    public void function(short numberOfLocals) throws Controller.ProgramException {
     ...
     */
	public void returnFromBuiltInFunction(short returnValue) throws Controller.ProgramException {
     ...
     */
    public void returnFromFunction() throws Controller.ProgramException {

        // make sure that there's somewhere to return to (old local <> 0)
        if (stackSegment.getValueAt(Utilities.Definitions.LOCAL_POINTER_ADDRESS) == 0)
            throw new Controller.ProgramException("Nowhere to return to in " +
        ...
        bus.send(ram, Utilities.Definitions.LOCAL_POINTER_ADDRESS, ram, Utilities.Definitions.R13_ADDRESS); // R13 = lcl
        bus.send(stackSegment, stackSegment.getValueAt(Utilities.Definitions.LOCAL_POINTER_ADDRESS) - 5, ram, Utilities.Definitions.R14_ADDRESS); // R14 = return address
        bus.send(stackSegment, getSP() - 1, stackSegment, ram.getValueAt(Utilities.Definitions.ARG_POINTER_ADDRESS)); // *arg = return value
        setSP((short)(ram.getValueAt(Utilities.Definitions.ARG_POINTER_ADDRESS) + 1)); // SP = arg + 1
        bus.send(stackSegment, ram.getValueAt(Utilities.Definitions.R13_ADDRESS) - 1, ram, Utilities.Definitions.THAT_POINTER_ADDRESS); // that = *(R13 - 1)
        bus.send(stackSegment, ram.getValueAt(Utilities.Definitions.R13_ADDRESS) - 2, ram, Utilities.Definitions.THIS_POINTER_ADDRESS); // this = *(R13 - 2)
        bus.send(stackSegment, ram.getValueAt(Utilities.Definitions.R13_ADDRESS) - 3, ram, Utilities.Definitions.ARG_POINTER_ADDRESS); // arg = *(R13 - 3)
        bus.send(stackSegment, ram.getValueAt(Utilities.Definitions.R13_ADDRESS) - 4, ram, Utilities.Definitions.LOCAL_POINTER_ADDRESS); // lcl = *(R13 - 4)
        ...
        if (stackFrames.size() > 0) {
            ...
            localSegment.setEnabledRange(Math.max(localSegment.getStartAddress(), Utilities.Definitions.STACK_START_ADDRESS), frameAddress - 1, true);
            ...
            thisSegment.setEnabledRange(Math.max(thisSegment.getStartAddress(), Utilities.Definitions.HEAP_START_ADDRESS), Utilities.Definitions.HEAP_END_ADDRESS, true);
            thatSegment.setEnabledRange(Math.max(thatSegment.getStartAddress(), Utilities.Definitions.HEAP_START_ADDRESS), Utilities.Definitions.SCREEN_END_ADDRESS, true);
        }/* else {
			error("Nowhere to return to");
		} */ // Allow return if we previously had "function" even with no call -
			 // For the SimpleFunction test

        short returnAddress = ram.getValueAt(Utilities.Definitions.R14_ADDRESS);
		if (returnAddress == VMProgram.BUILTIN_FUNCTION_ADDRESS) {
			...
			if (stackFrames.size() > 0) {
				...
				staticSegment.setStartAddress(Utilities.Definitions.VAR_START_ADDRESS);
				staticSegment.setEnabledRange(Utilities.Definitions.VAR_START_ADDRESS,
											  Utilities.Definitions.VAR_END_ADDRESS - 1,
											  true);
			}
	...
	public void callFunctionFromBuiltIn(String functionName, short[] params)
			throws Controller.ProgramException {
    ...
    public void callFunction(short address, short numberOfArguments, String functionName, boolean callerIsBuiltIn)
     throws Controller.ProgramException {
        ...
        pushFromRAM(MAIN_STACK, Utilities.Definitions.LOCAL_POINTER_ADDRESS);
        pushFromRAM(MAIN_STACK, Utilities.Definitions.ARG_POINTER_ADDRESS);
        pushFromRAM(MAIN_STACK, Utilities.Definitions.THIS_POINTER_ADDRESS);
        pushFromRAM(MAIN_STACK, Utilities.Definitions.THAT_POINTER_ADDRESS);
        ram.setValueAt(Utilities.Definitions.ARG_POINTER_ADDRESS, (short)(getSP() - numberOfArguments - 5), false);
        ram.setValueAt(Utilities.Definitions.LOCAL_POINTER_ADDRESS, getSP(), false);
     ...
     */
    protected void setStaticRange(String functionName) throws Controller.ProgramException {
        int dotLocation = functionName.indexOf(".");
        if (dotLocation == -1)
            throw new Controller.ProgramException("Illegal function name: " + functionName);

        String className = functionName.substring(0, dotLocation);
        int[] range = program.getStaticRange(className);
        if (range == null)
            throw new Controller.ProgramException("Function name doesn't match class name: " + functionName);
    ...
    private void calculate(int numberOfArgs, int operator) throws Controller.ProgramException {
    ...
    private void pushValue(int stackID, short value) throws Controller.ProgramException {
        short sp = getSP();
    ...
    private void pushFromRAM(int stackID, int index) throws Controller.ProgramException {
        short sp = getSP();
        bus.send(ram, index,
    ...
    private void pushFromCalculator(int stackID, int index) throws Controller.ProgramException {
        short sp = getSP();
    ...
    private void pushFromSegment(int stackID, short segmentCode, int index)
     throws Controller.ProgramException {
        short sp = getSP();
        ComputerParts.MemorySegment segment = (segmentCode == HVMInstructionSet.STATIC_SEGMENT_CODE) ?
    ...
    // and increments sp.
    private void popToSegment(int stackID, short segmentCode, int index) throws Controller.ProgramException {
	...
	// the popped value.
    private short popValue(int stackID) throws Controller.ProgramException {
        short newSP = (short)(getSP() - 1);
		short value;
    ...
    // and increments sp.
    private void popToRAM(int stackID, int index) throws Controller.ProgramException {
        short newSP = (short)(getSP() - 1);
        bus.send((stackID == MAIN_STACK ? stackSegment : workingStackSegment), newSP,
                 ram, index);
    ...
    // and increments sp.
    private void popToThisPointer(int stackID) throws Controller.ProgramException {
        short value = ram.getValueAt(getSP() - 1);
        if ((value < Utilities.Definitions.HEAP_START_ADDRESS || value > Utilities.Definitions.HEAP_END_ADDRESS)
            && value > 0)
            error("'This' segment must be in the Heap range");

        popToRAM(stackID, Utilities.Definitions.THIS_POINTER_ADDRESS);
        thisSegment.setEnabledRange(value, Utilities.Definitions.HEAP_END_ADDRESS, true);
    }

    // sends a value from the appropriate stack (at sp-1) to the that pointer
    // and increments sp.
    private void popToThatPointer(int stackID) throws Controller.ProgramException {
        short value = ram.getValueAt(getSP() - 1);
        if (!((value >= Utilities.Definitions.HEAP_START_ADDRESS && value <= Utilities.Definitions.HEAP_END_ADDRESS) ||
              (value >= Utilities.Definitions.SCREEN_START_ADDRESS && value <= Utilities.Definitions.SCREEN_END_ADDRESS) ||
              value == 0))
            error("'That' segment must be in the Heap or Screen range");

        popToRAM(stackID, Utilities.Definitions.THAT_POINTER_ADDRESS);
        thatSegment.setEnabledRange(value, Utilities.Definitions.SCREEN_END_ADDRESS, true);
    }

    // sends a value from the appropriate stack (at sp-1) to the calculator at the given index
    // and increments sp.
    private void popToCalculator(int stackID, int index) throws Controller.ProgramException {
        short newSP = (short)(getSP() - 1);
        bus.send((stackID == MAIN_STACK ? stackSegment : workingStackSegment), newSP,
                 calculator, index);
    ...
    private short popAndReturn() throws Controller.ProgramException {
        short newSP = (short)(getSP() - 1);
        checkSP(newSP);
        setSP(newSP);
     ...
     */
    public ComputerParts.MemorySegment getStaticSegment() {
     ...
     */
    public short getSP() {
        return ram.getValueAt(Utilities.Definitions.SP_ADDRESS);
    }
    ...
    public void setSP(short value) {
        ram.setValueAt(Utilities.Definitions.SP_ADDRESS, value, true);
    }

    // Checks the given sp value. If not legal, throws an exception.
    private void checkSP(short sp) throws Controller.ProgramException {
        if (sp < Utilities.Definitions.STACK_START_ADDRESS || sp > Utilities.Definitions.STACK_END_ADDRESS)
            error("Stack overflow");
    }

    // Verifies that the given index of the given segment is valid.
    private void checkSegmentIndex(ComputerParts.MemorySegment segment, int segmentCode, int index)
     throws Controller.ProgramException {
        short loc = (short)(index + segment.getStartAddress());

        if (segmentCode == HVMInstructionSet.THIS_SEGMENT_CODE) {
            if (loc < Utilities.Definitions.HEAP_START_ADDRESS || loc > Utilities.Definitions.HEAP_END_ADDRESS)
    ...
    private void error(String message) throws Controller.ProgramException {
        throw new Controller.ProgramException(message + " in " + callStack.getTopFunction() + "." +
